...
- [ ] **5. AI 教練與 RAG 顯示**
  ...
  - 預期：回答下方會出現「下一步建議」，引導使用者看來源、補充持倉資料、跑壓力測試或回饋回答品質。
  - 預期：有 `trace_id` 時會顯示 👍／👎 RAG 回饋；送出失敗時回答仍保留，可重試。
  - 預期：失敗時只顯示固定安全內容，不顯示 provider exception、traceback 或 API key。
  - 0830 差異：回答新增 trace、citation、confidence、下一步建議、RAG feedback 與安全降級紀錄。
...
- [ ] **8. 情境壓力測試**
  - 若帳戶只有現金，先點「比較四種情境」。
  - 預期：畫面提示「目前組合只有現金」，不顯示全 0 表格，也不送出壓力測試 API。
  - 點「載入示範投資組合（僅供展示）」或手動建立一筆小額虛擬買入。
  - 使用同一組 snapshot、期間與 seed 重跑相同策略／情境。
  - 預期：結果一致；可切換 3 種策略與 4 種情境。
  - 預期：表格旁顯示「結果解讀」，把黑天鵝總報酬、最大回撤與期末值轉成風險低／中／高和下一步建議。
  - 預期：測試前後模擬交易帳本不變。
  - 0830/0922 差異：新增可重現的合成壓力測試，並補上純現金提示、白話解讀與示範資料入口。
...
- **4:00–5:20｜展示模擬交易與新壓力測試**
  - 若帳戶只有現金，先展示純現金提示，再點「載入示範投資組合（僅供展示）」。
  - 快速展示既有虛擬持倉，再跑一組 black_swan。
  - 用同 seed 重跑，指出結果一致且帳本不變。
  - 指出「結果解讀」：風險低／中／高、主要原因與建議下一步。
  - 說：「0727 有模擬交易；0830/0922 新增的是獨立、可重現、看得懂的合成情境測試。」
