...
### 目前本地 Eval 結果（2026-07-27）

在未安裝 `jieba`、`rank-bm25`、`chromadb` 的本地驗證環境下，系統會啟用 dependency-light sparse fallback。修正中文 fallback tokenization 與 endpoint topic prior 後，15 筆標準題的檢索結果如下：
...
| NDCG@3 | 0.7768 |
| NDCG@5 | 0.8102 |
| Avg keyword overlap | 0.8900 |
| Method distribution | sparse fallback 15/15 |

> 說明：上述結果證明 RAG 知識庫與檢索流程在無向量依賴時仍可用。若部署環境完整安裝 `requirements.txt` 中的 `jieba`、`rank-bm25`、`chromadb` 並設定 OpenAI embedding key，系統會進一步啟用 BM25 / ChromaDB dense retrieval。
