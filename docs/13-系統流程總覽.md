...
> **MVP 現狀**：訂閱系統已提供方案查詢、訂閱查詢與 mock checkout，可寫入 `subscriptions` / `payment_records` 展示資料流。實際第三方金流驗證仍待 Phase 2 實作。
...
> **目前 MVP 為手動同步模式**。使用者在模擬交易頁面手動輸入買入/賣出，也可透過手動同步 API 儲存外部平台持倉快照；系統不會自動讀取任何真實交易所的持倉或交易紀錄。
