...
**對應功能**：訂閱管理。MVP 已提供 mock checkout 與訂閱查詢 API，可展示資料流；真實第三方金流驗證仍屬 Phase 2。

**程式對應**：`supabase_client.py` (`get_user_subscription`, `upsert_subscription`)、`app.py` (`/api/membership/subscription`, `/api/membership/mock-checkout`)
...
**程式對應**：`supabase_client.py` (`list_membership_plans`)、`app.py` (`/api/membership/plans`)
...
**對應功能**：金流紀錄。MVP 以 mock checkout 產生展示用付款紀錄；真實金流待 Phase 2 串接。

**程式對應**：`supabase_client.py` (`create_payment_record`)、`app.py` (`/api/membership/mock-checkout`)
...
**程式對應**：`app.py` (`/api/narratives`)、`supabase_client.py` (`save_narrative_signal`)
...
**程式對應**：`app.py` (`/api/scam-scan`)、`supabase_client.py` (`save_scam_scan_log`)
...
**程式對應**：`app.py` (`/portfolio/analyze-llm`, `/api/health/reports`)、`supabase_client.py` (`save_portfolio_health_report`, `list_portfolio_health_reports`)
...
> **目前 MVP 狀態**：所有外部帳號同步為**手動模式**。系統提供 `/api/external-sync/manual` 儲存手動持倉快照與同步紀錄；Phase 2 將透過 CCXT 統一介面串接交易所 API，實現自動同步。
...
> **MVP 現狀**：目前模擬交易支援一般、盤整市場、牛市、山寨輪動、熊市、黑天鵝六種情境。切換情境時，後端會依 `MARKET_SCENARIOS` 的價格乘數重估持倉市值，並將情境價格套用到本次模擬下單；前端同步顯示情境總資產、現金緩衝與本次下單影響。真實歷史壓力測試與跨策略績效比較留待 Phase 2。
...
> 本章資料庫設計與 `supabase_client.py` 中的 CRUD 方法及實際使用資料表已對應。subscriptions、membership_plans、payment_records、notifications、external_account_links 目前支援 MVP/mock/manual 資料流；真實金流與交易所 API 自動同步仍列為 Phase 2，不在 MVP 假裝完成。
