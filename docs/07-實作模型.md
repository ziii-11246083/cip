...
3. 過濾規則:
   ...
   - 標題過短、連結格式異常、來源缺失者剔除或標記 needs_review
4. 去重: 以文章 URL 為唯一鍵，24h 內重複文章合併
5. 驗證標記: 每筆資料附加 validation / verified / validation_status
  ↓
...
3. 過濾規則:
   ...
   - 移除與加密貨幣無關的泛科技新聞
4. 清洗: 移除 HTML tag、壓縮空白、檢查 HTTP/HTTPS 連結
5. 敘事分類: 關鍵字匹配 (BTC/ETH/DeFi/L2/Meme/RWA/AI/GameFi/Regulation)
  ↓
...
| PTT | 移除刪文、異常連結、重複 URL；保留真實推文數，不產生假互動數 |
| RSS/CNYES | 移除 HTML、驗證連結、去重；不再用 random 產生推文數 |
| 情緒標記 | 改用 bullish/bearish 關鍵字規則，不再隨機標示 BULLISH/BEARISH |
