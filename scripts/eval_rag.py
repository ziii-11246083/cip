...
def ndcg_at_k(retrieved_topics: List[str], expected_topics: List[str], k: int) -> float:
    ...
    # Binary relevance by unique expected topic. Without de-duplication, repeated
    # chunks from the same topic can make DCG exceed IDCG.
    dcg = 0.0
    matched = set()
    for i, t in enumerate(retrieved_topics[:k]):
        if t in expected_topics and t not in matched:
            matched.add(t)
